...
class LiveAssemblyCompilerApp:
    ...
    def setup_ui(self):
        ...
        # Input text box
        self.cpp_input = tk.Text(self.root, wrap="word", height=15, bg="black", fg="white", insertbackground="white")
        self.cpp_input.pack(fill="both", expand=True, padx=5, pady=(5, 0))
        self.cpp_input.bind("<KeyRelease>", self.compile_code)
        
        # Output text box
        self.asm_output = tk.Text(self.root, wrap="word", height=15, state="disabled", bg="black", fg="white")
        self.asm_output.pack(fill="both", expand=True, padx=5, pady=(0, 5))
    ...
    def compile_code(self, event=None):
        ...
        # Command to compile to assembly with Intel syntax
        try:
            asm_output = subprocess.check_output(
                [self.compiler_var.get(), cpp_file_path, "-S", "-o", "-", self.optimization_var.get(), "-masm=intel"],
